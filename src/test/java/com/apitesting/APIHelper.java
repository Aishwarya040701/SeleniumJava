...
public class APIHelper {
	...
	public Object getUsersGeneric(String apiName, String jsonData) {
		Users users = null;
		Posts posts = null;
		try {
			if (apiName.equals("UsersApi")) {

				users = objectMapper.readValue(jsonData, Users.class);

				return users;

			} else if (apiName.equals("PostsApi")) {
				posts = objectMapper.readValue(jsonData, Posts.class);
				return posts;
			}
		} catch (JsonMappingException e) {
	...
	public String getJsonDataGeneric(Object userData) {
		String jsonData = null;
		try {
			jsonData = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(userData);
		} catch (JsonProcessingException e) {
